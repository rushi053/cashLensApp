...
> **Unshipped relative to store 2.1:** everything on `release/2.2` — the `PurchaseIntent` listener for promoted In-App Purchases (`4f702fa`), `Marketing/PromoImages/`, and the 2.2 Phase 0 / Phase A commits (version bump, `AppConstants.supportEmail`, `ReviewPromptManager`, paywall truth-pass, `sidebarAdaptable` tabs, size classes, `NavigationStack`, adaptive grids, width-derived chart heights).  
...
Repository root (162 Swift files on `release/2.2`; 161 on `redesign/v2`, plus the Xcode project, marketing assets, and a legacy `website/` folder). The tracked `CashLens.xcodeproj/xcuserdata/…/xcschememanagement.plist` that `redesign/v2` carried despite `.gitignore` was removed on `release/2.2` (`be7a28b`).
...
| `AllExpensesView` | `AllExpensesView.swift` (Activity tab) | Mounted with `isRootTab: true`; skips its `NavigationStack` wrapper in that mode (sheet / deep-link presentations get one). Full description under *Lists & Search* below. |
...
| `ReviewPromptManager` | `ReviewPromptManager.swift` | Rating-ask timing only (no UI); replaced `FeedbackManager` in 2.2 (`8de55d1`). Asks **once per marketing version** (`reviewPromptAskedVersion`) at a delight moment: the save that reaches `expenseThreshold = 10` expenses or a no-spend streak reaching `streakThreshold = 7` days. Never during onboarding, never while a paywall is visible, never within `sheetQuietInterval = 2 s` of a sheet dismiss. Not `@MainActor` (triggers come from `ExpenseViewModel`); publishes `shouldRequestReview`, and `MainTabView` calls the native `requestReview` action. Apple's 3-per-365-days throttle is the backstop. Store 2.1 shipped the older `FeedbackManager` rule (12 actions across 3+ days or 5 after an export, 30-day cooldown, max 3). |
...
**Adaptivity (state on `release/2.2`):** `.sidebarAdaptable` tab shell; `horizontalSizeClass` instead of `userInterfaceIdiom` for layout (the one remaining `userInterfaceIdiom` read is copy-only, in `PrivacyDashboardView`); `NavigationStack` everywhere (no `NavigationView` left); `GridItem(.adaptive(minimum:))` for the category / app-icon / theme pickers; `.adaptiveHeight` charts; heatmap cells follow Dynamic Type. Still to do per the 2.2 plan: readable `maxWidth` columns, two-column Today / Insights, Activity list + detail, `verticalSizeClass` for onboarding / paywall, retire `tabBarInset`, `systemExtraLarge` widgets, keyboard shortcuts / hover, iPhone Duo items. Store 2.1 had none of this (stretched phone layout on iPad; see the adaptivity audit).  
...
| 2026-09-19 | **2.2 (8)** in progress on `release/2.2` | Phase 0 landed: `be7a28b` bump to 2.2 (8) + drop tracked `xcuserdata`; `7289d22` tip-jar purchase intents; `9c9d840` paywall truth-pass; `8de55d1` `ReviewPromptManager`; `4810369` `AppConstants.supportEmail`. Phase A started: `0691176` `sidebarAdaptable` + size-class FAB; `f3e9942` size classes replace `userInterfaceIdiom`, `.adaptiveHeight`; `bb47fde` `NavigationView` → `NavigationStack`; `7a4e291` adaptive picker grids; `da8a529` width-derived chart heights + Dynamic Type heatmap cells. Remaining 2.2 scope in `PRO_FEATURES.md` → Version Plan. |

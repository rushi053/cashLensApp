...
Repository root (165 Swift files on `release/2.2`; 161 on `redesign/v2`, plus the Xcode project, marketing assets, and a legacy `website/` folder). The tracked `CashLens.xcodeproj/xcuserdata/…/xcschememanagement.plist` that `redesign/v2` carried despite `.gitignore` was removed on `release/2.2` (`be7a28b`).
...
│   │                               #   NavigationSplitView list + detail editor on regular width (2.2)
...
│   ├── Theme.swift                 # Spacing (incl. scrollBottomClearance), Radius, Stroke, Typography, Shadow, Motion, Icon, appDuotone
...
| `MainTabView` | `MainTabView.swift` | Root shell with **4 tabs — Today / Activity / Insights / You** (`enum Tab`). Two code paths: iOS 26+ `modernTabView` uses native `TabView` + `SwiftUI.Tab` with `.tabViewStyle(.sidebarAdaptable)` (2.2 — regular width such as iPad gets a sidebar automatically; compact keeps the system floating tab bar), `.tint(.appPrimary)`; iOS 18–25 `legacyTabView` draws a custom 60 pt tab bar with `UITabBar.appearance().isHidden = true`. `FloatingAddButton` overlay on every tab except You and while Activity is in bulk-select mode; its size and padding are driven by `horizontalSizeClass` (`isRegularWidth`) since 2.2 — the `userInterfaceIdiom` / `width > 768` checks from `redesign/v2` are gone. The FAB stays an overlay (not a toolbar item) because the tab roots deliberately have no `NavigationStack`, so there is no toolbar to host "+"; it sits on the `TabView`'s safe-area frame so it cannot enter a Duo vertical bar. The iOS 26 `TabView` also carries `.duoTabBarCompression()` (no-op today, §11). The legacy bar publishes `.safeAreaPadding(.bottom, tabBarHeight + safeAreaInsets.bottom)` so tab roots clear it via the safe area. Keyboard: `handleKeyboardCommand` receives `AppCommandCenter` commands (guards App Lock + onboarding): Cmd-N opens the add sheet, Cmd-, selects You, Cmd-F selects Activity then re-emits `.presentActivitySearch` after 0.25 s so a freshly mounted Activity root can hear it. The You tab root is unmounted when hidden (`youTabRoot`) to stop Profile's environment fan-out from stuttering later tab switches. Hosts: `.saveErrorBannerHost()`, `.saveConfirmationToastHost()`, post-value auto-paywall (`PaywallTrigger`, `PaywallView(context: .insights)`), `DonorThanksView` and `WinBackView` sheets (flags consumed on `onAppear`), native `requestReview` driven by `ReviewPromptManager.shouldRequestReview`, add-expense and currency-picker sheets. |
...
| `ReviewPromptManager` | `ReviewPromptManager.swift` | Rating-ask timing only (no UI); replaced `FeedbackManager` in 2.2 (`8de55d1`). **Policy:** ask **once per marketing version** (`reviewPromptAskedVersion` = `CFBundleShortVersionString`) at a delight moment — `recordExpenseSaved(totalExpenseCount:)` when the count reaches `expenseThreshold = 10` (called from `ExpenseViewModel+CRUD.addExpense`, with `nil` when only the launch hot-window count is known so the threshold is never trusted early), or `recordStreak(days:)` when a no-spend streak reaches `streakThreshold = 7` (called from `TodayView`'s streak recompute, guarded: hydrated, streak `isMeaningful`, an expense in the last 30 days — `StreakCalculator` reports a long "streak" for an empty ledger). **Presentation rules:** `canAsk` requires onboarding complete and not yet asked for this version; `arm()` → `schedule()` fires `sheetQuietInterval = 2 s` after the later of now and the last `noteSheetDismissed()` (called by `MainTabView` when the add sheet closes), so the ask lands on a settled screen; `paywallDidAppear()` cancels the scheduled fire and `fireIfStillAllowed()` refuses while `isPaywallVisible` — the 10th-expense auto-paywall takes the moment and the ask stays pending for the next save; `paywallDidDisappear()` does **not** re-arm (asking right after a closed paywall reads as pushy). The version flag is written only when `shouldRequestReview` actually publishes; `consume()` resets it. Not `@MainActor` (triggers arrive from `ExpenseViewModel`); `MainTabView` observes `$shouldRequestReview`, calls `consume()` and the native `requestReview` action, and skips (without retry) if App Lock is up. `resetForDebugging()` is wired to `DiagnosticsView`. Apple's 3-per-365-days throttle is the backstop. Store 2.1 shipped the older `FeedbackManager` rule (12 actions across 3+ days or 5 after an export, 30-day cooldown, max 3). |
...
|--------|---------|---------|
| `Theme.Spacing` | `xxs(2)`, `xs(4)`, `sm(8)`, `md(12)`, `lg(16)`, `xl(20)`, `xxl(24)`, `xxxl(32)`, `scrollBottomClearance(32)` | Rhythm for padding and `VStack` / `HStack` spacing. `tabBarInset(100)` was **deleted in 2.2** (`46b7404`): scroll roots now keep a 32 pt clearance and the legacy custom bar publishes its height through `.safeAreaPadding`, so content clears the bar via the safe area instead of a magic pad |
...
Insights' 2-column layout keys off `horizontalSizeClass`; Today has no grid.
...
- Nothing is keyed on orientation or `userInterfaceIdiom` (the one remaining idiom read in `PrivacyDashboardView` is copy-only), so a fold that changes only the size class cannot pick a different screen. Expected fold behaviour: `MainTabView` state survives the bar ↔ sidebar swap; Activity's sheets are attached outside `activityNavContainer` and survive the split ↔ stack swap while the editor moves between sheet and detail column (unsaved field edits do not carry over; child state such as the embedded calendar month resets); onboarding `currentStep` and paywall `selectedPlan` persist while their layout flips on `verticalSizeClass`.

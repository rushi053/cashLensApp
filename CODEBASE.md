...
> **Branches:** `main` is stale at 1.0.5 (2026-01-21, 70 Swift files, 3 tabs). `redesign/v2` (161 Swift files, 4 tabs) is what store 2.1 shipped from. `release/2.2` is cut from it and is where 2.2 is being built (166 Swift files, 45 commits ahead at `774a659`, 2026-09-19, incl. nine launch-readiness review fixes, eight perf quick wins + a debug seeder, and four pass-2 review fixes). This document describes `release/2.2`; where a fact changed between `redesign/v2` and `release/2.2` it says so.  
...
│   └── DiagnosticsView.swift       # #if DEBUG data health tools + stress-seeder buttons; presented from You → Developer (DEBUG only)
│
├── Components/
...
| `DiagnosticsView` | `DiagnosticsView.swift` | `#if DEBUG` only. Data refresh, currency checks, smoke tests, "Reset Review Prompt State", and (2.2) the stress-seeder buttons — "Seed 5,000 test expenses" / "Seed 20,000 test expenses" behind a confirmation dialog, "Delete seeded data" (destructive, confirmed), a status line, and a footnote naming the `-CLSeedExpenses <n>` launch argument (see §23). Since `6719371` it is presented as a sheet from the `#if DEBUG` "Developer (debug builds only)" group at the bottom of the You tab (`ProfileView.developerSection`, row "Diagnostics" — its only entry point; nothing is compiled into Release). |
...
| `AppCommandCenter` / `AppCommands` | `AppCommands.swift` | 2.2 hardware-keyboard support (iPad, Duo with a keyboard). `AppCommands: Commands` is attached once via `WindowGroup { … }.commands { AppCommands() }` in `CashLensApp` and declares a `CommandGroup(after: .newItem)` with `Label`s (so the iPadOS Cmd-hold HUD names them): **Cmd-N** New Expense, **Cmd-F** Search Expenses, **Cmd-,** Settings. Each button sends an `AppCommandCenter.Command` (`newExpense`, `searchActivity`, `presentActivitySearch`, `openSettings`) through a `PassthroughSubject` (not `@Published`, so late subscribers never replay); `MainTabView` and `AllExpensesView` subscribe. **Esc** is not declared here. `SheetCloseButton(action:respondsToEscape:)` (`Components/SheetHeader.swift`) applies `.keyboardShortcut(respondsToEscape && !escapeOwnedByPresentation ? KeyboardShortcut.cancelAction : nil)`, so every custom-chrome sheet closes through its own dismiss path (including `AddExpenseView.onDismissRequest`) and only the **topmost presentation** answers Esc. Two mechanisms feed that (`5121e1a`, `0922f23`): (1) `SheetHeader(escapeClosesSheet:)` (default `true`) — every presenter with sub-sheets passes its own "nothing above me" condition: `AddExpenseView` `!hasPresentationAbove` (scanner, receipt viewer, receipt paywall, templates, manage categories, category picker, date picker, field editor), `AddSubscriptionView` (category picker, date picker, manage categories), `BudgetSetupView` (category picker, Pro paywall), `BudgetListView` (add / edit budget, paywall), `CustomCategoryForm` (icon / color picker), `AppearanceStudioView` (paywall), `PrivacyDashboardView` (export sheet), `NotificationsSettingsView` (schedule sheets), `QuickSearchView` (`selectedExpense == nil`); (2) the `EnvironmentValues.escapeOwnedByPresentation` key (`EscapeOwnedByPresentationKey`, default `false`) — a container that presents sheets above content which itself carries a `SheetCloseButton` sets it `true` into that content's environment: `MainTabView` sets it to `hasPresentation` on both tab paths, and `AllExpensesView` sets `inheritedEscapeOwned || hasOwnPresentation` (Quick Search, calendar, date range, tag paywall, bulk pickers) on the inline detail-pane editor so the editor never steals Esc from a sheet above the ledger. Lone close buttons (Paywall, Recap, Summary) keep the default. The root Done / Cancel buttons of Export, Import, ManageCategories, Diagnostics, About, AppIconPicker and SiriShortcutsTips carry `.keyboardShortcut(.cancelAction)` directly. |
...
Insights' 2-column layout (`StatisticsView.usesTwoColumns`) requires regular width **and** a measured content width ≥ `twoColumnMinimumWidth = 640` (`onGeometryChange` on the `ScrollView`, `d33ab47`) — a 50/50 iPad split is regular but would give two columns narrower than an iPhone SE. Before the first measurement (`measuredContentWidth == 0`) it trusts the size class alone (`774a659`) so an iPad does not flash one column for a frame. Padding and the 1200 pt cap still key off `isWideLayout` alone. Today has no grid.
...
### Debug Tools
- `DiagnosticsView` (`#if DEBUG`): You tab → "Developer (debug builds only)" → Diagnostics (entry point restored in 2.2, `6719371`; it had none since the v2 IA rework). Hosts data refresh, currency checks, smoke tests, review-prompt reset and the stress-seeder buttons.
...
| `602b5b4`, `774a659` | `TodayInsight.pick` compares against precomputed `[todayStart, todayEnd)` instead of `isDate(inSameDayAs:)` per row; day-range ends are wrapped in `startOfDay` so a DST transition at midnight yields the same membership | `TodayView` |
| `319af89`, `d26c541`, `774a659` | Activity day grouping computes day bounds once per group (end wrapped in `startOfDay`); per-row equality fallback kept | `AllExpensesView` recompute |
...
- **Marker:** every seeded row has `subscriptionId == 0000C1EE-5EED-4000-8000-000000000000` (`sentinelSubscriptionID`) with `isFromSubscription == false`, so nothing treats it as a real subscription expense. Six custom categories are created once with the exact names in `seededCategoryNames` (`Seed Coffee`, `Seed Games`, `Seed Pets`, `Seed Tech`, `Seed Self Care`, `Seed Events`).
...
- **Entry points:** `DiagnosticsView` (itself `#if DEBUG`, reached from You → Developer (debug builds only) → Diagnostics) with "Seed 5,000 / 20,000 test expenses" behind a confirmation dialog, "Delete seeded data" (destructive, confirmed) and a status line; and the launch argument **`-CLSeedExpenses <n>`**, read in `CashLensApp.init` via `UserDefaults.standard.integer(forKey: "CLSeedExpenses")` only when the store loaded, run synchronously before the view models hydrate so cold start can be measured against a full table, and idempotent (skips when the store already holds ≥ n seeded rows).
- **Delete path:** `deleteSeededData` runs two `NSBatchDeleteRequest`s — `subscriptionId == <sentinel>` on `ExpenseEntity` and `name IN seededCategoryNames` on `CustomCategoryEntity` (exact list; `774a659` replaced the earlier `BEGINSWITH "Seed "` so a real user category such as "Seed Money" can never be deleted) — merges the deleted IDs into the view context and posts `.expensesChangedExternally`. Nothing else is touched.

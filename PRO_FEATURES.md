...
| **2.2 (8)** | **In progress on `release/2.2`** — single build with everything below | **Landed (Phase 0, 2026-09-19):** version 2.2 / build 8 and the tracked `xcuserdata` plist dropped (`be7a28b`); promoted-IAP listener now also routes tips (`7289d22`; promo images still to be uploaded in App Store Connect before submission); paywall truth-pass across the three tiers (`9c9d840`); `ReviewPromptManager` once-per-version rating ask (`8de55d1`); unified support email via `AppConstants` (`4810369`); these docs. **Landed (Phase A, partial):** `sidebarAdaptable` tab shell + size-class FAB (`0691176`); size classes replace `userInterfaceIdiom`, `.adaptiveHeight` modifier (`f3e9942`); `NavigationView` → `NavigationStack` everywhere (`bb47fde`); adaptive category / icon / theme grids (`7a4e291`); width-derived chart heights + `@ScaledMetric` heatmap cells (`da8a529`). **Remaining:** readable `maxWidth` columns, two-column Today / Insights, Activity list + detail on regular width, `verticalSizeClass` for onboarding / paywall, retire `tabBarInset`, `systemExtraLarge` widgets; **iPad polish** (keyboard shortcuts, hover, multitasking sizes); **iPhone Duo prep** on the iOS 27 SDK (toolbar titles + symbols, safe-area compliance, even-column grids, fold-transition state); per-device screenshot test suite. Candidate: multi-page receipts. Built with release Xcode 27. | `release/2.2` (cut from `redesign/v2`), one PR to `main` |
